...
class LaporanController extends Controller
{
    ...
    public function laporan_cetak(Request $request) {
        ...
        $dataLaporan = Pembayaran::rightJoin('siswa', function ($join) use ($request) {
            $join->on('siswa.id', 'pembayaran.id_siswa')
            ->where('pembayaran.bulan_dibayar', $request->bulan_spp);
        })
        ->leftJoin('spp', function ($join) use ($request) {
            $join->on('spp.id', 'pembayaran.id_spp')
            ->where('spp.kode_spp', $request->spp);
        })
        ->select('pembayaran.tanggal_bayar', 'pembayaran.bulan_dibayar', 'siswa.nama AS nama_siswa')
        ->where('siswa.kode_kelas', $request->kelas)
        ...
        if (count($dataLaporan) > 0) {
            ...
            foreach($dataLaporan as $index => $result){
                ...
                $this->fpdf->Cell(80, 6, $result->nama_siswa, 1, 0, 'L');
                $this->fpdf->Cell(55, 6, $result->tanggal_bayar ? strftime('%d %B %Y', strtotime($result->tanggal_bayar)) : "-", 1, 0, 'L');
                $this->fpdf->SetFont('Times', 'B', 11);
                if ($result->tanggal_bayar != "") {
